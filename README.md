...
## What a video cannot fake: depth

A photograph is a rigid plane, and the deformation signal above kills it. A
*video* of you — including one generated by a model — deforms and blinks exactly
like you do, so that signal calls it live. Two later checks answer the video
case instead, both geometric:

**Homography residual.** Whatever a flat panel shows, however the content
animates, its landmarks move between frames under a single homography. A head has
volume: nose, ear and contour sit at different depths, so rotating it produces
parallax no homography can absorb. VAKT fits a normalised-DLT homography over the
whole landmark set on rotated frame pairs and measures the leftover error per
radian of rotation. On synthetic tests a plane scores 0.0000 px at every angle,
while a landmark cloud with depth scores 0.99, 3.95 and 7.35 px at 2°, 8° and 15°.
Below the floor with enough rotated samples, the verdict is `screenReplay`.

**Background rigidity.** A phone or print held in the hand carries its background
with it. VAKT registers patches outside the face box between frames and compares
their translation with the face's own; sustained agreement while the face is
clearly moving means one rigid object, and the verdict is `heldDevice`. It is
vetoed once head depth is proven, because a nudged desk also moves face and room
together.

**The sensor itself.** A deepfake is likelier to arrive through a virtual camera
than through a phone at the lens, and no optical signal can question frames that
never came from a scene. External, Continuity and virtual cameras are refused, and
the camera `uniqueID` is recorded at enrolment.

Honest limit: fusing these raises the cost of an attack a lot, but passive RGB
stays probabilistic. A 4K video on a large panel, brightness matched, at the right
distance, with little head rotation, can still get through. Only the active
challenge on the roadmap is categorical.
...
| `screenReplay` | Matching face whose geometry is provably flat — a display |
| `heldDevice` | Face and background moving as one rigid object |
...
Done since 0.1.0 — see `CHANGELOG.md`: planar replay detection via homography
residual, background rigidity, and camera pinning against virtual cameras.

1. **Active liveness.** Modulate the light our own screen emits with a random
   sequence and correlate it against the face crop's luma — lock-in detection. A
   real face reflects it; a display does not. The only signal here that a
   prerecorded or generated video cannot answer, because the challenge is created
   at verification time. Intrusive, so it should run only when another signal is
   already in doubt.
2. **Spectral signature.** FFT over the face crop: displays leave moiré banding
   and prints have an unnaturally flat high-frequency response. Fails on a
   high-density display at distance, so fusion only.
